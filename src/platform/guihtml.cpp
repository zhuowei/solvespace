...
public:
    ...
    MenuItemRef AddItem(const std::string &label, std::function<void()> onTrigger) override {
        std::shared_ptr<MenuItemImplHtml> menuItem = std::make_shared<MenuItemImplHtml>();
        menuItems.push_back(menuItem);
        menuItem->onTrigger = onTrigger;
        val::global("window").call<void>("setLabelWithMnemonic", menuItem->htmlMenuItem, Wrap(label));
