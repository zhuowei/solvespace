...
- (SSFunction *)initWithFunction:(std::function<void ()> *)timer;
- (void)run;
@end
...
- (SSFunction *)initWithFunction:(std::function<void ()> *)_func {
    if(self = [super init]) {
        func = _func;
    }
    return self;
...
public:
    ...
    MenuItemImplCocoa() {
        ...
        [nsMenuItem setTarget:ssFunction];
    }
    ...
    void SetAccelerator(KeyboardEvent accel) override {
        ...
        switch(accel.key) {
            ...
            case KeyboardEvent::Key::FUNCTION:
                accelChar = NSF1FunctionKey + accel.num;
                break;
        }
        [nsMenuItem setKeyEquivalent:
            [[NSString alloc] initWithCharacters:&accelChar length:1]];
    ...
    void SetActive(bool active) override {
        [nsMenuItem setState:(active ? NSOnState : NSOffState)];
    }

    void SetEnabled(bool enabled) override {
        [nsMenuItem setEnabled:enabled];
    }
};
